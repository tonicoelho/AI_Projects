...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            # Ghost is captured; update beliefs to put all probability on
            # the jail position
            ...
            for p in self.legalPositions:
                ...
                newBeliefs[p] = self.beliefs[p] * emissionModel[trueDistance]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            positionIndex = i % numLegalPositions
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            # Ghost is captured; update beliefs to put all probability on
            # the jail position
            newParticles = [self.getJailPosition() for _ in self.particles]
        else:
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        positions = [self.legalPositions for _ in range(self.numGhosts)]
        allTuples = list(itertools.product(*positions))
        # evenly distribute particles
        for i in range(self.numParticles):
            particleIndex = i % len(allTuples)
            self.particles.append(allTuples[particleIndex])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(gameState, i, self.ghostAgents[i])
